 ...
 */
public abstract class AggregatorOp<T, U, V> implements AggregableReduceOp<T, U, V> {
    ...
    public static <T, U, V, W, X> AggregatorOp<T, Pair<U, W>, Pair<V, X>> compose(final AggregatorOp<T, U, V> thisAggregator, final AggregatorOp<T, W, X> otherAggregator) {
    ...
    public <W> AggregatorOp<T, U, W> andFinally(final Function<V, W> finalize) {
        return new AggregatorOp<T, U, W>(){

            @Override
            public U tally(U accumulator, T element) {
                return AggregatorOp.this.tally(accumulator, element);
            }

            @Override
            public U combine(U accu1, U accu2) {
                return AggregatorOp.this.combine(accu1, accu2);
            }

            @Override
            public U neutral() {
                return AggregatorOp.this.neutral();
            }

            @Override
            public W summarize(U acc) {
                return finalize.apply(AggregatorOp.this.summarize(acc));
            }
        };
    }
